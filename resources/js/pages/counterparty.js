 ...
 *
 * Its own case, never a kind of "outstanding". The sign is the whole meaning —
 * showing −₹5,000 in the amber that everywhere else means "chase this" would
 * send somebody after money the workshop is *holding*, not owed.
 */
function isAdvance(amount) {
    return amount !== null && amount !== undefined
        && !isZeroAmount(amount) && String(amount).trim().startsWith('-');
}
...
function render() {
    ...
    $('#parties-body').innerHTML = pageRows.length
...
function renderStats() {
    ...
    $('#stat-settled').textContent = (counts.settled + counts.advance).toLocaleString('en-IN');
    $('#stat-due').textContent = counts.due.toLocaleString('en-IN');
    $('#stat-recent').textContent = scope.filter(addedRecently).length.toLocaleString('en-IN');
...
function renderSummary(matched) {
    ...
    $('#parties-summary').textContent = parts.join(' ');
}

function renderPager() {
    const host = $('#parties-pager');
...
function renderSortIndicators() {
    $$('#parties-head [data-sort]').forEach((th) => {
...
function openForm(party = null) {
    ...
    const { noun } = state.config;

    clearFormErrors(form);
    form.reset();

    $('#party-modal-title').textContent = editing ? `Edit ${party.name}` : `Add New ${cap(noun)}`;

    $('#party-modal-subtitle').textContent = editing
        ? 'Changing the GSTIN moves which columns of a tax return their bills land in.'
        : `Enter the ${noun}'s contact and business details.`;

    $('#party-submit').textContent = editing ? 'Save changes' : `Add ${noun}`;
    ...
    // A new record starts on the role of the screen it was created from; an
    // existing one shows what it actually holds, so the "also a vendor" case
    // stays editable from either list.
    const roles = editing ? (party.roles ?? []) : [state.config.role];
...
function showStateHint(gstin) {
    ...
    $('#party-state-hint').textContent = /^\d{2}$/.test(code)
...
async function submitForm(event) {
    ...
    try {
        ...
        toast(editing ? `${cap(state.config.noun)} updated.` : `${cap(state.config.noun)} added.`);
        ...
        // Dropping this screen's role sends the record to the other list. Say
        // so, rather than letting it vanish and look like a failed save.
        if (!roles.includes(state.config.role)) {
            toast(`They are no longer a ${state.config.noun}, so they have moved to the `
                + `${state.config.otherRole}s screen.`, 'info');
        }
...
function renderSortPanel() {
    $('#sort-panel').innerHTML = sortOptions()
...
export function initCounterpartyPage(config) {
    return async function init() {
        ...
        $('#parties-body').innerHTML = tableMessage(COLUMNS, `Loading ${config.nounPlural}…`);
        ...
        try {
            ...
            // workshop, so they can reach this page by typing the URL and there
            // is nothing to show them. Not their mistake — say so plainly.
            $('#parties-body').innerHTML = error.code === 'NO_WORKSPACE'
                ? tableMessage(COLUMNS, 'Your account administers the platform rather than a single workshop, '
                    + `so it has no ${config.nounPlural} of its own.`)
        ...
        $('#parties-head').addEventListener('click', (event) => {
            const th = event.target.closest('[data-sort]');

            if (th) applySort(th.dataset.sort);
        });

        $('#parties-pager').addEventListener('click', (event) => {
        ...
        $('#parties-body').addEventListener('click', (event) => {
        ...
        $('#parties-body').addEventListener('keydown', (event) => {
